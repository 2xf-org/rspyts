...
Reports contract links Catalog registrations, but keeps Catalog-owned
...
The consuming `reports/rspyts.toml` uses this strict dependency mapping:
...
2. Retain only root-owned functions, resources, errors, and constants as local
   exports. A dependency's `define_signal` must never appear in Reports.
...
- It is acceptable for `example.reports.SignalDefinition` to re-export
...
- Generate codecs against the imported type and its verified schema; do not
  synthesize an Reports-owned DTO or conversion graph.
- Register only root-owned native functions/resources in Reports's PyO3
  module.
...
cargo run -p rspyts-cli -- check --locked --config examples/cross-package/catalog/rspyts.toml
cargo run -p rspyts-cli -- check --locked --config examples/cross-package/reports/rspyts.toml
