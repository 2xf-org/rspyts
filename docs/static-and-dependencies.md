...
Suppose Hardware defines `SignalDefinition`, and Evaluation returns an
`EvaluationResult` containing it. Do not copy that model into Evaluation and
maintain conversions in three languages.

rspyts instead records the Cargo package that owns every registered Rust type.
Evaluation can use Hardware's real Rust type directly. Its generated packages
then import Hardware's generated host type.

The generated Evaluation packages import `SignalDefinition` from
`neurovirtual.hardware` and `@neurovirtual/hardware`. They do not depend on
Hardware's `.rspyts/` staging directory. The lock verifies Rust contract
compatibility at compile time; it does not pin released Python or npm versions.

### 1. Define the owner package

Hardware owns the Rust type and its host package names:

```toml
# examples/cross-package/hardware/rspyts.toml
[crate]
path = "rust"
features = ["bindings"]
...
Lock Hardware before a dependent package tries to resolve it:

```sh
rspyts lock \
  --config examples/cross-package/hardware/rspyts.toml

rspyts build \
  --config examples/cross-package/hardware/rspyts.toml
```

The generated Hardware package owns the host models. Abridged:

```python
# neurovirtual/hardware/models.py
...
```ts
// @neurovirtual/hardware/index.d.ts
...
Evaluation has a normal Cargo dependency on Hardware and refers to its real Rust
type:
...
Its rspyts configuration maps the foreign Cargo owner to the committed Hardware
lock and the package names that consumers install:

```toml
# examples/cross-package/evaluation/rspyts.toml
[crate]
path = "rust"
...
`hardware` is a local alias. `crate` must match the Cargo package owner captured
when `SignalDefinition` was registered; `lock` is relative to Evaluation's
configuration.

At compile time, rspyts reads Hardware's lock, verifies the referenced type and
fingerprint, and records the dependency's owner, host names, fingerprint, and
referenced shape in Evaluation's lock. That shape is verification data, not a
second generated model.

### 3. Build the consumer

With Hardware's current lock available:

```sh
rspyts lock \
  --config examples/cross-package/evaluation/rspyts.toml

rspyts build \
  --config examples/cross-package/evaluation/rspyts.toml
```

Evaluation generates only the model it owns. This abridged Python model module
imports Hardware's class:

```python
# neurovirtual/evaluation/models.py
...
```python
from neurovirtual.evaluation import SignalDefinition as EvaluationSignal, evaluate
from neurovirtual.hardware import SignalDefinition as HardwareSignal

assert EvaluationSignal is HardwareSignal

signal = HardwareSignal(id="sig:c3", sample_rate_hz=256)
result = evaluate(signal, 1)

assert isinstance(result.signal, HardwareSignal)
assert result.accepted is True
```

Evaluation's TypeScript declarations use the installed Hardware package too:

```ts
import type { SignalDefinition } from "@neurovirtual/hardware";
export type { SignalDefinition } from "@neurovirtual/hardware";
...
There is no copied `interface SignalDefinition` in Evaluation. Because this
fixture configures TypeScript as `static`, Evaluation's Rust `evaluate`
...
rspyts emits `"@neurovirtual/hardware": "*"` in Evaluation's npm
...
packager must declare the compatible Hardware distribution.
...
If Hardware changes `SignalDefinition`, update packages in dependency order:

```sh
# 1. Accept and build the owner.
rspyts lock --config examples/cross-package/hardware/rspyts.toml
rspyts build --config examples/cross-package/hardware/rspyts.toml

# 2. Re-resolve, accept, and build the consumer.
rspyts lock --config examples/cross-package/evaluation/rspyts.toml
rspyts build --config examples/cross-package/evaluation/rspyts.toml
