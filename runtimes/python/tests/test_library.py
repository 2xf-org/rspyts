...
@pytest.mark.parametrize(
    ("name", "expected"),
    [
        ("neurovirtual_hardware", "RSPYTS_LIBRARY_NEUROVIRTUAL_HARDWARE"),
...
def test_library_specific_env_vars_allow_multiple_libraries(monkeypatch, tmp_path):
    hardware_path = tmp_path / "hardware.dylib"
    evaluation_path = tmp_path / "evaluation.dylib"
    monkeypatch.setenv("RSPYTS_LIBRARY_NEUROVIRTUAL_HARDWARE", str(hardware_path))
    monkeypatch.setenv("RSPYTS_LIBRARY_NEUROVIRTUAL_EVALUATION", str(evaluation_path))

    assert Library("neurovirtual_hardware").resolve_path() == hardware_path
    assert Library("neurovirtual_evaluation").resolve_path() == evaluation_path
