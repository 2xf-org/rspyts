"""ABI 3 schema codecs used through :mod:`rspyts.internal`."""
...
BUFFER_DTYPES: dict[str, type[np.generic]] = {
    "u8": np.uint8,
    "i8": np.int8,
    "u16": np.uint16,
    "i16": np.int16,
...
def response_value(value: object) -> envelope.Response:
...
def bool_from_wire(response: envelope.Response) -> bool:
    ...
    value = response_value(response).value
...
def bytes_from_wire(response: envelope.Response) -> bytes:
    ...
    return typing.cast(bytes, envelope.materialize_attachment(response_value(response), "bytes"))
...
def buffer_from_wire(response: envelope.Response, *, dtype: str) -> np.ndarray:
    ...
    expected = BUFFER_DTYPES.get(dtype)
    if expected is None:
        raise ValueError(f"rspyts: unsupported buffer dtype {dtype!r}")
    value = envelope.materialize_attachment(response_value(response), dtype)
...
def i64_from_wire(response: envelope.Response) -> int:
    ...
    return exact_int(response_value(response).value, signed=True, allow_host_int=False)


def u64_from_wire(response: envelope.Response) -> int:
    """Validate a canonical unsigned 64-bit wire string."""
    return exact_int(response_value(response).value, signed=False, allow_host_int=False)
...
def checked_json_value(value: typing.Any, active: set[int]) -> typing.Any:
    ...
    try:
        if value_type is list:
            return [checked_json_value(item, active) for item in value]
        for key in value:
            if type(key) is not str:
                raise TypeError("schemaless JSON object keys must be exact strings")
        return {key: checked_json_value(item, active) for key, item in value.items()}
    finally:
...
def json_to_wire(value: typing.Any) -> typing.Any:
    ...
    return checked_json_value(value, set())
...
def json_from_wire(response: envelope.Response) -> typing.Any:
    ...
    return checked_json_value(response_value(response).value, set())
