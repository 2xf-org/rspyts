...
from . import errors, library  # noqa: F401
from .models import AnalysisParams, HardwareInfo
...
"#,
    ...
    fn mapped_foreign_types_are_imported_not_emitted() {
        ...
        assert!(
            models.contains(
                "from neurovirtual.hardware.generated import HardwareInfo  # noqa: F401\n"
            ),
            "{models}"
        );
        assert!(!models.contains("class HardwareInfo"), "{models}");

        // __init__ re-exports it exactly like a local model.
        let init = &files.iter().find(|(n, _)| *n == "__init__.py").unwrap().1;
        assert!(init.contains("HardwareInfo"), "{init}");

        // classes.py keeps importing it via .models, which re-exports.
        let classes = &files.iter().find(|(n, _)| *n == "classes.py").unwrap().1;
        assert!(
            classes.contains("from .models import AnalysisParams, HardwareInfo"),
            "{classes}"
        );
    }
    ...
    fn unmapped_foreign_types_are_emitted_locally() {
        ...
        assert!(models.contains("class HardwareInfo(Contract):"), "{models}");
        assert!(!models.contains("neurovirtual"), "{models}");
    }
    ...
    fn mapped_foreign_error_enums_are_imported_not_emitted() {
        ...
        assert!(
            errors.contains(
                "from neurovirtual.hardware.generated import AnalysisError, \
    ...
    fn foreign_types_referenced_by_local_models_need_no_noqa() {
        ...
        let models = &files.iter().find(|(n, _)| *n == "models.py").unwrap().1;
        assert!(
            models.contains("from neurovirtual.hardware.generated import HardwareInfo\n"),
            "{models}"
        );
        assert!(!models.contains("HardwareInfo  # noqa"), "{models}");
    }
