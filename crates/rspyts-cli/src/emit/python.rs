...
mod tests {
    ...
    fn source_mode_preserves_shared_namespace_while_standalone_initializes_it() {
        ...
        for (staging, package, marker) in [
            (&catalog, "example.catalog.contracts", "catalog"),
            (
                &services,
                "example.services.contracts",
                "services",
            ),
        ] {
            let parent = staging
        ...
        let python_path =
            std::env::join_paths([catalog.join("python"), services.join("python")]).unwrap();
        let import = Command::new("python3")
            ...
            .arg(
                "import example.catalog as catalog; \
                 ...
                 assert catalog.PACKAGE_MARKER == 'catalog'; \
                 assert services.PACKAGE_MARKER == 'services'",
            )
            .output()
        ...
        let config = PythonConfig {
            package: "example.catalog.contracts".into(),
            source: None,
        ...
        .unwrap();
        assert!(standalone.join("python/example/__init__.py").is_file());
        assert!(
            standalone
                .join("python/example/catalog/__init__.py")
                .is_file()
        );
        assert!(
            standalone
                .join("python/example/catalog/contracts/__init__.py")
                .is_file()
        );
        fs::remove_dir_all(root).unwrap();
    }
    ...
    fn foreign_models_are_imported_from_the_configured_python_package() {
        let catalog_owner = rspyts::ir::CargoPackageId::new("catalog");
        let catalog_identity =
            rspyts::ir::DefinitionId::new(catalog_owner.as_str(), "catalog::SignalDefinition");
        let foreign = TypeDef {
            owner: catalog_owner.clone(),
            id: catalog_identity.id.clone(),
        ...
        let contract = ResolvedContract {
            ...
            dependencies: BTreeMap::from([(
                ...
                crate::LockedDependency {
                    owner: catalog_owner,
                    fingerprint: "sha256:catalog".into(),
                    python: Some("example.catalog.contracts".into()),
                    typescript: None,
                    types: vec![foreign.clone()],
                    errors: vec![],
                },
            )]),
            foreign_types: BTreeMap::from([(catalog_identity, foreign)]),
            foreign_errors: BTreeMap::new(),
        };
        let generated = models(&contract, &type_names(&contract));

        assert!(generated.contains("from example.catalog.contracts import SignalDefinition"));
