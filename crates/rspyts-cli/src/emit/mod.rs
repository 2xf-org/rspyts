...
//! longer emits, but only `.py`/`.ts` files whose opening header carries the
//! rspyts marker — user files that stray into an out dir are never
//! touched.
...
pub fn plan(cfg: &Config, manifest: &Manifest, hash: &str) -> Plan {
    ...
    if let Some(py) = &cfg.python {
        ...
        let rust_source = rust_source_reference(&py.out, &cfg.crate_dir);
        let provenance = util::Provenance {
            manifest_hash: hash,
            rust_source: &rust_source,
        };
        add(
            &py.out,
            python::emit(manifest, &provenance, &library_search, &py.imports),
        );
    }
    if let Some(ts) = &cfg.typescript {
        let rust_source = rust_source_reference(&ts.out, &cfg.crate_dir);
        let provenance = util::Provenance {
            manifest_hash: hash,
            rust_source: &rust_source,
        };
        add(
            &ts.out,
            typescript::emit(manifest, &provenance, &ts.imports),
        );
    }
    if let Some(sc) = &cfg.schema {
        let rust_source = rust_source_reference(&sc.out, &cfg.crate_dir);
        let provenance = util::Provenance {
            manifest_hash: hash,
            rust_source: &rust_source,
        };
        add(&sc.out, schema::emit(manifest, &provenance));
    }
    Plan { files, owned }
}

/// A portable path from one generated directory to the bridged Rust source.
fn rust_source_reference(out: &Path, crate_dir: &Path) -> String {
    pathdiff::diff_paths(crate_dir.join("src"), out)
        .map(|path| path.to_string_lossy().replace('\\', "/"))
        .unwrap_or_else(|| "rspyts.toml:[crate].path/src".to_string())
}
...
/// Conservative: only files whose opening header carries the rspyts marker
...
fn stray_generated_files(owned: &OwnedDir) -> Result<Vec<PathBuf>> {
    ...
    for entry in entries {
        ...
        if content
            .lines()
            .take(8)
            .any(|line| line.contains(util::GENERATED_MARKER))
        {
            strays.push(path);
        }
    }
...
mod tests {
    ...
    fn plan_covers_all_configured_out_dirs() {
        ...
        for file in plan.files.iter().filter(|file| {
            ...
            assert!(file.content.contains("DO NOT EDIT THIS FILE"));
            assert!(
                file.content
                    .contains("Edit the Rust source tree instead: ../crate/src")
            );
            assert!(!file.content.contains("/virtual"));
        }
        let schema = plan
            .files
            .iter()
            .find(|file| file.path == root.join("schema/schema.json"))
            .expect("schema output");
        assert!(schema.content.contains("DO NOT EDIT THIS FILE"));
        assert!(schema.content.contains("\"rustSource\": \"../crate/src\""));
        assert!(!schema.content.contains("/virtual"));
    }
