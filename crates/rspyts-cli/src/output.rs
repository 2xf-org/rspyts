    ...
    for source_root in ["src-py", "src-ts"] {
        ...
        for path in files.keys() {
            ...
            if source_root == "src-py" && is_native_binary(path) {
                let base = relative
                    .strip_suffix(".so")
                    .or_else(|| relative.strip_suffix(".pyd"))
                    .context("Python native module has an unsupported extension")?;
                paths.insert(format!("{base}.pyd"));
                paths.insert(format!("{base}.so"));
            } else {
...
mod tests {
    ...
    fn generated_state_separates_python_typescript_and_native_files() {
        ...
        write(&generated.path().join("src-py/pkg/native.so"), "native\n").unwrap();
        write(&generated.path().join("src-ts/pkg/api.ts"), "typescript\n").unwrap();
        write(
            &generated.path().join("src-ts/pkg/native_bg.wasm"),
            "wasm\n",
        )
        .unwrap();
